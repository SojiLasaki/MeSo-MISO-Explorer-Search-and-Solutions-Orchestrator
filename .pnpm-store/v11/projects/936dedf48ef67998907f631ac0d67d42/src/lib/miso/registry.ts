import { CATALOG_OPERATIONS, operationToSource } from "./catalog";
import { LEGACY_REPORT_SOURCES } from "./legacy-reports";
import type { MisoSource } from "./types";

const REPORT_SOURCES: MisoSource[] = [
  {
  ...
  },
  {
    ...
    type: "webpage",
    description:
      "Official Data Exchange catalog of Load, Generation, Interchange and Pricing APIs.",
    supports: [
      "api documentation",
      "how to authenticate",
      "subscription key",
...
export function scoreSource(question: string, source: MisoSource): number {
  ...
  if (/\bmcp\b|ancillary|asm\b/.test(q) && (path.includes("asm") || path.includes("mcp")))
    score += 6;
  if (
    /\b(actual load|yesterday'?s? load|real-?time load|daily power usage|power usage)\b/.test(q) &&
    path.includes("demand/actual")
  ) {
    score += 6;
  }
  if (
    /\bcleared demand\b/.test(q) &&
  ...
  if (/\bhourly\b/.test(q) && (source.parameters ?? []).some((p) => p.name === "timeResolution"))
    score += 1;
  return score;
}

export function rankSources(question: string): RankedSource[] {
  const q = question.toLowerCase();
  const wantsReport = /\b(report|workbook|pdf|imm)\b/.test(q) && !/\bapi\b/.test(q);
  const wantsLegacyReport =
    /\b(?:old(?:er)?|legacy|archiv(?:e|ed)|retired)\b[\s\w-]{0,48}\b(?:report|reports|file|files)\b|\breader'?s guide\b|\breport[- ]to[- ]endpoint\b|\breport mapping\b/i.test(
      question,
    );
  const wantsDocs =
    /\b(documentation|developer portal|subscription key|authenticate|data exchange)\b/.test(q);

  if (wantsDocs) {
    const docs = getSource("miso_api_documentation");
    return docs ? [{ source: docs, score: 20 }] : [];
  }
  ...
  if (wantsReport) {
    const monthly = /\bmonthly|imm\b/.test(q);
    const report = getSource(monthly ? "miso_market_report_monthly" : "miso_market_report_daily");
    return report ? [{ source: report, score: 20 }] : [];
  }

  return MISO_SOURCES.map((source) => ({ source, score: scoreSource(question, source) }))
    .filter((row) => row.score > 0)
    .sort((a, b) => b.score - a.score);
}

export function matchSource(question: string): MisoSource | null {
  const ranked = rankSources(question);
  const best = ranked[0];
  return best && best.score >= 3 ? best.source : null;
}

export function isAmbiguousRanking(ranked: RankedSource[]): boolean {
  const first = ranked[0];
  const second = ranked[1];
  if (!first || !second) return false;
  if (first.score < 4 || second.score < 4) return false;
  return second.score >= first.score * 0.8;
}
