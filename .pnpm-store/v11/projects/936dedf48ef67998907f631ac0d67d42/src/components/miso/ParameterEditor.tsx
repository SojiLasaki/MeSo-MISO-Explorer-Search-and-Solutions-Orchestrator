...
export function ParameterEditor({
  ...
  if (!specs.length) {
    return (
      <p className="text-[13px] text-muted-foreground">This source doesn't take any parameters.</p>
    );
  }
      ...
      {specs.map((spec) => {
        ...
        return (
          <div key={spec.name} className="space-y-1.5">
            ...
            {missing && <p className="text-[12px] text-destructive">{spec.label} is required.</p>}
          </div>
        );
      })}
    </div>
