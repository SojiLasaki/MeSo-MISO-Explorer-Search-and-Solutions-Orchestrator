import { $ as WORKFLOW_DESERIALIZE, B as postFormDataToApi, Et as UnsupportedFunctionalityError, J as serializeModelOptions, N as isCustomReasoning, O as generateId, S as createNullLanguageModelUsage, St as InvalidResponseDataError, V as postJsonToApi, W as resolveFullMediaType, X as withUserAgentSuffix, Z as withoutTrailingSlash, _ as createEventSourceResponseHandler, _t as union, at as array, b as createJsonResponseHandler, c as StreamingToolCallTracker, dt as looseObject, et as WORKFLOW_SERIALIZE, f as combineHeaders, gt as string, h as convertToFormData, ht as record, it as any, j as getTopLevelMediaType, m as convertToBase64, mt as object, ot as boolean, p as convertBase64ToUint8Array, pt as number, tt as _enum, ut as literal, w as downloadBlob, wt as TooManyEmbeddingValuesForCallError, x as createLanguageModelResponseMetadata, xt as InvalidPromptError, y as createJsonErrorResponseHandler, z as parseProviderOptions } from "./@ai-sdk/gateway+[...].mjs";
							...
							case "file": switch (part.data.type) {
								...
								case "data": {
									...
									if (topLevel === "video") return {
										type: "video_url",
										video_url: { url: part.data.type === "url" ? part.data.url.toString() : `data:${resolveFullMediaType({ part })};base64,${convertToBase64(part.data.data)}` },
										...partMetadata
									};
									if (topLevel === "audio") {
...
var openAICompatibleContentSchema = union([string(), array(looseObject({ type: string() }))]).nullish();
